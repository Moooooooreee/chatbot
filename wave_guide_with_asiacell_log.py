...
ASIACELL_FILE = "AsiaCell_empty.xlsx"


# Logging function to write to AsiaCell Excel
def log_to_asiacell(date, imei, technician="Unknown", status="Fixed", work_done="waveguide installed", notes="", spectro_actions="", problem="", count="Unique"):
    try:
        df = pd.read_excel(ASIACELL_FILE, dtype={"IMEI": str})
        new_row = {
            "Visit date": date,
            "IMEI": imei,
            "Work done": work_done,
            "Status": status,
            "Notes": notes,
            "Spectro actions": spectro_actions,
            "Technician": technician,
            "Problem": problem,
            "Count": count
        }
        df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
        df.to_excel(ASIACELL_FILE, index=False)
    except Exception as e:
        print(f"Failed to write to AsiaCell log: {e}")
